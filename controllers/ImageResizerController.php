...
class ImageResizerController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $image   = "{$dir}{$file}";
        $to_save = rand(10000, 999999);
        $to_save = "{$dir}{$to_save}{$file}";

        echo '<pre>';
        var_dump(self::resizer($image, $to_save, '1920', '350'));
    }

    public static function resizer($path, $save, $width, $height)
    {
        //$width = $height * 5;
        ...
        if ($src_aspect <
            $thumb_aspect) {        //узкий вариант (фиксированная ширина)      $scale = $width / $size[0];         $new_size = array($width, $width / $src_aspect);        $src_pos = array(0, ($size[1] * $scale - $height) / $scale / 2); //Ищем расстояние по высоте от края картинки до начала картины после обрезки   } else if ($src_aspect > $thumb_aspect) {
        ...
        imagecopyresampled($thumb, $src, 0, 0, $src_pos[0], $src_pos[1], $new_size[0], $new_size[1], $size[0],
            $size[1]);
